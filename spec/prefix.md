这是deepseek关于缓存优化的官方文档，根据行业惯例，缓存命中的 tokens 通常会有显著的价格折扣
...
这是deepseek关于缓存优化的官方文档, 已经成为行业标准其他厂商也通用
实践建议
稳定前缀尽量前置：将 system prompt 和固定的 context 放在 messages 最前面，变化的用户问题放在最后，这样前缀越长、越稳定，收益越大。

避免在前缀中插入动态变量：哪怕是时间戳、随机数等微小变化，也可能导致整个前缀匹配失败。

首次请求有“预热”成本：缓存构建需要秒级时间，首次请求或缓存失效后的请求，可能感受到额外的延迟。